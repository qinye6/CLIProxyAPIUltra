...
func NewAntigravityAuth(cfg *config.Config, httpClient *http.Client) *AntigravityAuth {
	...
	if cfg != nil && cfg.UpstreamEndpoints.AntigravityBaseURL != "" {
		apiEndpoint = cfg.UpstreamEndpoints.AntigravityBaseURL
	}
...
func (o *AntigravityAuth) FetchProjectID(ctx context.Context, accessToken string) (string, error) {
	...
	endpointURL := fmt.Sprintf("%s/%s:loadCodeAssist", o.apiEndpoint, APIVersion)
...
func (o *AntigravityAuth) OnboardUser(ctx context.Context, accessToken, tierID string) (string, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		endpointURL := fmt.Sprintf("%s/%s:onboardUser", o.apiEndpoint, APIVersion)
